...
def generate_umaps(output_folder_path:str, config_path_umap:str, config_path_subset:str)->None:
    ...
    # CHANGED: 
    input_dir = f"/home/projects/hornsteinlab/giliwo/NOVA_rotation/embeddings/embedding_output"
    emb_dir = os.path.join(input_dir,"embeddings", config_umap.EXPERIMENT_TYPE)
    ...
    for batch in batches_names:
        ...
        samples_path = os.path.join(input_dir,"pairs", "euclidean", config_umap.EXPERIMENT_TYPE, batch)
        for marker in input_markers:
            ...
            saveroot = os.path.join(output_folder_path, config_umap.EXPERIMENT_TYPE, batch, marker)
            os.makedirs(saveroot, exist_ok=True)
            logging.info(f'saveroot: {saveroot}')
            
            # Calculate the UMAP embeddings for all samples
            embeddings, labels, paths  = load_embeddings(input_dir, config_umap)
            ...
            samples_indices = __extract_indices_to_plot(keep_samples_dir=temp_samples_path, paths = [paths], data_config = config_umap)
...
if __name__ == "__main__":
    ...
    try:
        ...
        config_path_umap = sys.argv[2]
        config_path_subset = sys.argv[3]

        generate_umaps(output_folder_path, config_path_umap, config_path_subset)
